...
if ((matches[1]:lower() == "adminprom" or matches[1] == 'ادمین ربات' and is_mod(msg))) and is_sudo(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
local text = matches[2]
tdcli.createNewChannelChat(text, 1, '@fire021tm', (function(b, d) tdcli.addChatMember(d.id_, msg.from.id, 0, dl_cb, nil) end), nil)
   if not lang then 
...
if ((matches[1]:lower() == "fire" or matches[1] == 'فایر' and is_mod(msg))) then
return tdcli.sendMessage(msg.to.id, msg.id, 1, _config.info_text, 1, 'html')
    end
if ((matches[1]:lower() == "adminlist" or matches[1] == 'لیست ادمین ربات' and is_mod(msg))) and is_admin(msg) then
...
◾️*ادمین* `[username|id|reply]`
﹃﹄﹃﹄﹃﹄﹃﹄﹃﹄﹃﹄
✩》*!admindem* `[username|id|reply]`
◾️*حذف ادمین* `[username|id|reply]`
﹃﹄﹃﹄﹃﹄﹃﹄﹃﹄﹃﹄
✩》*!adminlist *
◾️*لیست ادمین *
﹃﹄﹃﹄﹃﹄﹃﹄﹃﹄﹃﹄
